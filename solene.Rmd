...
model_TWO$results
model_TWO_knn$results

model_TWO$results$Accuracy
model_TWO_knn$results$Accuracy


plot(model_TWO_knn)
#the plot shows us that the higher the k the more accurate 
```

 
LOGISTIC MODEL WITH RAW DATA 
```{r}
set.seed(123)
train_index_zeros <- sample(1:nrow(data), 0.8 * nrow(data))
train_data_zeros <- data[train_index_zeros, ]
test_data_zeros <- data[-train_index_zeros, ]
...
#for now it gives the same because my data is cleaned, when we put into main it should work and give a worst version than the previous one 
